...
void loop() {
  ...
  //read_status();
  //read_io();
  showingImage();           // Send image from camera to Display
  display_menu();           // Show menu line on the bottom - comment to remove the menu
  param_adjust_run();       // Adjust parameters of camera from the menu
  update_setting();         // Transfer the updated configuration to camera
  read_status();            // Read Status of the charger IC
  config_sys();             // Write power down configuratino to charging IC
  //read_battery();         
